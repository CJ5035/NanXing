...
namespace NanXingCangKu.Utils
{
    class InStockHelper
    {
        ...
        public DataTable In(int count)
        {
            ...
            {
                Logger.Default.Process(new Log("Warn", $"没有空列入库"));
                return null;
            }
            else
            {
                ...
                count = count > dt.Rows.Count ? dt.Rows.Count : count;
                //分数量
                ...
                foreach (DataRow temp in newDt2.Rows)
                {
                    Debug.WriteLine(temp["lie"].ToString() + temp["xuhao"].ToString());
                }
                int carCount = 1;
                maxCount = 0;
                //分批
                ...
                //foreach (DataRow temp in newDt3.Rows)
                //{
                //    Debug.WriteLine(temp["lie"].ToString() + temp["xuhao"].ToString());
                //}
        ...
        private DataTable GetNullLie()
        {
            string sql = string.Format(@"
              select LEFT(WareLocaNo,charindex('-',WareLocaNo,charindex('-',WareLocaNo)+1)) lie,
                ...
                where a.WareArea_ID=b.id  and charindex('-',a.WareLocaNo)>0 and WareLocaState>0 and
                b.War_ID=2 and b.WareHouse_ID=3 and
                not exists(
             ...
             (RunState is null or (RunState!='已完成' and RunState!='已取消')))A
            ...
            ");
            DbHelperSQL.connectionString = ConfigurationManager.ConnectionStrings["WMS"].ToString();
