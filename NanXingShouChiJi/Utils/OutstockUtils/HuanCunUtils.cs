...
{
    public class HuanCunUtils
    {
        public async Task<DataTable> In1(int count, string position)
        {
            return await Task.Run(() => In(count, position));
        }
        #region 进缓存区2
        public DataTable In(int count, string position)
        {
            //1、得到空列与数量
            DataTable dt = GetNullLie(position);
            //int count2 = 0;
        ...
        /// </summary>
        /// <returns></returns>
        private DataTable GetNullLie(string position)
        {
            //and WareHouse_ID=3
