...
namespace NanXingService_WMS.Managers
{
    public class CRMPlanManager
    {
        ...
        /// <returns>结果</returns>
        public async Task<bool> SaveCRMFile(string CRMID, string fileName, System.Web.HttpRequest request)
        {
            //保存到文件夹，并保存在数据库中
            try
            {
                if (request.ContentLength > 0)
                {
                    ...
                    Task<int> readTask = request.InputStream.ReadAsync(buffer, 0, buffer.Length);
                    ...
                    //CoverJavaByteToString(buffer, ref text, 0);
                    text = Encoding.UTF8.GetString(buffer);
                    FileStream fstream = null;

                    //sbyte[] sb=ToJavaBytes(buffer);
                    fstream = File.Create(path, buffer.Length);
