...
namespace NanXingService_WMS.Services
{
    public class SensorDataService
    {
        ...
        private SensorData ParseData(FloorBase floor)
        {
            ...
            float[] ammeterResult = DataCommand(floor.Ammeter, floor);
            data.Ammeter1 = ammeterResult[0];
            data.Ammeter2 = ammeterResult[1];
            data.Ammeter3 = ammeterResult[2];

            float[] humitureReslut = DataCommand(floor.Humiture, floor);
            data.Humidity = humitureReslut[0];
            data.Temperature = humitureReslut[1];

            data.Noise = DataCommand(floor.Noise, floor);
            data.ToxicGas = DataCommand(floor.ToxicGas, floor);

            data.StackTemp1 = DataCommand(floor.Thermostat1, floor);
            data.ChamberTemp1 = DataCommand(floor.Thermostat2, floor);
            data.StackTemp2 = DataCommand(floor.Thermostat3, floor);
            data.ChamberTemp2 = DataCommand(floor.Thermostat4, floor);
            data.StackTemp3 = DataCommand(floor.Thermostat5, floor);
            data.ChamberTemp3 = DataCommand(floor.Thermostat6, floor);
            if (floor.Floor == "3")
        ...
        /// 发送报文获取设备数据
        /// </summary>
        /// <typeparam name="T">由于温湿度返回两个数据，用字符串返回，其余返回浮点数，所以使用泛型方法</typeparam>
        /// <param name="sensor">传感器接口对象</param>
        /// <returns></returns>
        private T DataCommand<T>(ISensor<T> sensor, FloorBase floor)
        {
            floor.sensorMsgFloor = string.Empty;

            try
            {
                StringBuilder stringBuilder = new StringBuilder();
                foreach (var item in sensor.CommandCode)
                {
                    stringBuilder.Append(item.ToString("x2"));
                }
                Logger.Default.Process(new Log(LevelType.Info,
                    $"322:发送\r\n{sensor.ToString()}:{stringBuilder}"));
                floor.sensorClient.client.Send(sensor.CommandCode);
                Thread.Sleep(200);
            }
            catch { }
            Logger.Default.Process(new Log(LevelType.Info, $"313:收到{floor.sensorMsgFloor}"));

            return sensor.GetResult(floor.sensorMsgFloor);
        }
        #endregion
        ...
        private void sensorFloor_Received(object sender, TcpDatagramReceivedEventArgs<byte[]> e)
        {
            lock (e.TcpClient.Client.RemoteEndPoint)
            {
                ...
                {
                    StringBuilder stringBuilder = new StringBuilder();
                    foreach (var item in e.Datagram)
                    {
                        stringBuilder.Append(item.ToString("x2"));
                    }
                    Logger.Default.Process(new Log(LevelType.Info, $"{iPEndPoint.Address}{iPEndPoint.Port}\r\n{stringBuilder}"));
                    ParseFloor(floor, e.Datagram);
                }
                catch (Exception ex)
                {
