...
namespace NanXingWMS_old.Utils.TCP
{
    public class SocketClientManager:IDisposable
    {
        ...
        /// 连接的终点
        /// </summary>
        public EndPoint _endPointendPoint = null;
        /// <summary>
        /// 消息对象
        /// </summary>
        ...
        /// <param name="outtime">检测心跳时间秒数</param>
        public SocketClientManager(string ip, int port,int checkTime=60,int retryTime=6)
        {
            this.retryTime = retryTime;
            this.CommuuicationOutTim = checkTime;
            this.Intwhile = 0;
        ...
        private void ClientAuthenticationAndReconnection()
        {
            ...
            if (BoolVerifyHeartbeatPacket == false)
            {
                ...
                for (int i = 0; i < retryTime && this._isConnected == false; i++)
                {
                    if (_socket == null)
                    {
        ...
        private  void ReceiveCallback(IAsyncResult ar)
        {
            ...
            using (this.socketInfo = ar.AsyncState as SocketInfo)
            {
                ...
                {
                    string strMsg = Encoding.ASCII.GetString((this.socketInfo.buffer)).Replace("\0", "");
        ...
        {
            if (_socket.Connected && !string.IsNullOrEmpty(msg))
            {
