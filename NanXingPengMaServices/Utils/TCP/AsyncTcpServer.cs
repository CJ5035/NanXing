...
namespace NanXingWMS_old.Utils.TCP
{
    /// <summary>
    /// 异步TCP服务器
    /// </summary>
    public class AsyncTcpServer : IDisposable
    {
        ...
        public void Send(TcpClient tcpClient, byte[] datagram)
        {
            lock (new object())
            {
                ...
                {
                    if (ex.ToString().IndexOf("强迫关闭") >= 0)
                    {
                        TcpClientState tcpstate = RemoveTcpClient(tcpClient);
                        if (tcpstate.MsgConn == 1)
                        {
                            this.clients.Remove(RemoveTcpClient(tcpClient));
                        }
                        else
                        {
        ...
        private TcpClientState RemoveTcpClient(TcpClient tcpClient)
        {
            TcpClientState tcpstate = null;
            for (int i = 0; i < this.clients.Count; i++)
            {
                if (this.clients[i].TcpClient.Client.RemoteEndPoint == tcpClient.Client.RemoteEndPoint)
                {
                    tcpstate = this.clients[i];
                }
            }
            return tcpstate;
        }
        ...
        {
            if (!IsRunning) throw new InvalidProgramException("This TCP server has not been started.");

            for (int i = 0; i < this.clients.Count; i++)
            {
                if (this.clients[i].TcpClient.Connected)
                {
