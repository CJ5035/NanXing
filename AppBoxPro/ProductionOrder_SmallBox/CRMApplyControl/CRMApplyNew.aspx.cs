    ...
    public partial class CRMApplyNew : PageBase
    {
        ...
        private void LoadData()
        {
            ...
            foreach (string temp in idStrs)
            {
                ...
                string keyCount = redisKeyStart + ":CRMADDCount:" + temp;
                foreach (string poTemp in positionArr)
                {
                    ...
                    if (redisHelper.KeyExists(keyCount))
                    {
                        redisHelper.KeyDelete(keyHead);
                    }
                }
            }
        ...
        private void BindGrid1()
        {
            ...
            qlist.ForEach((item)=>{
                ...
                ProPlanOrderlists.Itemno = item.ItemNo;
                ...
                ProPlanOrderlists.Spec = item.Spec;
                ...
                ProPlanOrderlists.Remark = item.Remark;
                proList.Add(ProPlanOrderlists);

                item.BoxNo = string.Empty;
                item.BoxName = string.Empty;
                //redisHelper.StringSet($"{redisKeyStart}:CRMADDCount:{item.ID}", item.OrderCount);
            });
        ...
        protected void Btn_SaveExit_Click(object sender, EventArgs e)
        {
            ...
            string ipAddress = GetLocalIp();
            redisKeyStart = $"{ipAddress}{pcy}";
            List<int> xuHaoList = new List<int>();
            string[] temp2 = CheckBoxList2.SelectedValueArray;
            ...
            for (int index=0;index<q.Count;index++)
            {
                ...
                //        ($"{redisKeyStart}:CRMADDCount:{q[index].ID}");

                redisKey_List = $"{redisKeyStart}:ADDOrderList";
                string key = redisKey_List + $":{q[index].ID}";
                List<ProPlanOrderlists> ProPlanOrderlists
                    = redisHelper.HashValues<ProPlanOrderlists>(key);
                decimal count = 0;
                var In= ProPlanOrderlists.Where(u => tempAll.Contains(u.Chejianclass)).ToArray();
                foreach (var temp in In)
                    count += temp.PcCount??0;

                if (count > q[index].OrderCount)
                    xuHaoList.Add(index + 1);
            }
            if (xuHaoList.Count<=0)
            {
                Save();
            }
            else
            {
                string indexStr = string.Join(",", xuHaoList);
                PageContext.RegisterStartupScript(Confirm.GetShowReference(
                    $"序号{indexStr}下推的小包装排产数大于申请单排产数，是否继续保存？",
                    String.Empty,
        ...
        private void Save()
        {
            ...
            for (int i = 0; i < q.Count; i++)
            {
                ...
                ProPlanOrderheaders.Jingbanren = GetIdentityChineseName();
